...
<div class="container">
    ...
    @endif
    <form action="{{ route('teachers.store') }}" method="post" class="shadow p-3 mb-5 bg-white rounded">
        @csrf
        <div class="row">
            <div class="col-md-6">
                <div class="form-group">
                    <label for="name">Teacher ID</label>
                    <input type="number" name="teacher_id" class="form-control @error('teacher_id') is-invalid @enderror" value="{{old('teacher_id')}}" required>
                    @error('teacher_id')
